...
static uint8_t mcp9600_read_register(uint8_t ubAddr, uint8_t ubReg)
{
    ...
    {
        i2c0_write_byte(MCP9600_I2C_ADDR(ubAddr), ubReg, I2C_STOP);
    //}
    ...
    //ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
    //{
        ubVal = i2c0_read_byte(MCP9600_I2C_ADDR(ubAddr), I2C_STOP);
    }
...
static uint16_t mcp9600_read_register16(uint8_t ubAddr, uint8_t ubReg)
{
    ...
    //ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
    //{
        i2c0_read(MCP9600_I2C_ADDR(ubAddr), ubBuf, 2, I2C_STOP);
    }
...
static uint32_t mcp9600_read_register24(uint8_t ubAddr, uint8_t ubReg)
{
    ...
    //ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
    //{
        i2c0_read(MCP9600_I2C_ADDR(ubAddr), ubBuf, 3, I2C_STOP);
    }
...
static void mcp9600_write_register(int8_t ubAddr, uint8_t ubReg, uint8_t ubVal)
{
    uint8_t ubBuf[2];
    ...
    ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
    {
        i2c0_write(MCP9600_I2C_ADDR(ubAddr), ubBuf, 2, I2C_STOP);
    }
...
static void mcp9600_write_register16(uint8_t ubAddr, uint8_t ubReg, uint16_t usVal)
{
    ...
    ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
    {
        i2c0_write(MCP9600_I2C_ADDR(ubAddr), ubBuf, 3, I2C_STOP);
    }
...
uint8_t mcp9600_init(uint8_t ubAddr)
{
    ...
    ATOMIC_BLOCK(ATOMIC_RESTORESTATE)
    {
        if(!i2c0_write(MCP9600_I2C_ADDR(ubAddr), 0, 0, I2C_STOP)) // Check ACK from the expected address
            return 0;
    }
